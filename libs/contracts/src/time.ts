...
export function parseTimeToMs(input: string): number | null {
  ...
  // 백분초: 한 자리면 ×10 (".3" → 300ms), 두 자리면 ×10 (".34" → 340ms)
  const fracStr = m[4] ?? '';
...
/** 향상(개선)이면 음수, 느려지면 양수. 단위 ms. */
export function timeDeltaMs(previousMs: number, currentMs: number): number {
